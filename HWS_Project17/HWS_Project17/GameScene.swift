...
//  HWS_Project17
//
//  Created by Roman Kravtsov on 27/01/2019.
...
class GameScene: SKScene {
    ...
    func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        if randomPosition.x < 256 {
            randomXVelocity = Int.random(in: outsideMultiplier)
        } else if randomPosition.x < 512 {
            randomXVelocity = Int.random(in: middleRange)
    ...
    func tossEnemies() {
        ...
        case .chain:
            ...
            for i in 1...enemiesInChain - 1 {
                DispatchQueue.main.asyncAfter(deadline: .now() + (chainDelay / 5.0 * Double(i))) { [ unowned self ] in self.createEnemy() }
            }
        ...
        case .fastChain:
            ...
            for i in 1...enemiesInChain - 1 {
                DispatchQueue.main.asyncAfter(deadline: .now() + (chainDelay / 10.0 * Double(i))) { [ unowned self ] in self.createEnemy() }
            }
        }
